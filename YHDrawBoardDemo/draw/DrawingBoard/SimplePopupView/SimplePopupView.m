...
@interface SimplePopupView () <UITableViewDelegate,UITableViewDataSource>
...
@property (nonatomic, strong) UIView *addOneView;//cell添加一个View
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        switch (direction) {  //根据箭头方向,tableView位移并改变size
            case PopViewDirectionTop:
                ...
                view.viewOrigin = CGPointMake(view.frame.origin.x, view.frame.origin.y+edgeWidth*cos(M_PI/3));
                view.viewSize = CGSizeMake(view.frame.size.width, view.frame.size.height-edgeWidth*cos(M_PI/3));
                if ((self.trianglePercent*view.frame.size.width < edgeWidth*cos(M_PI/3))||((1-self.trianglePercent)*view.frame.size.width < edgeWidth*cos(M_PI/3)))
                {
                    _isMargin = YES;
                }
                break;
            case PopViewDirectionLeft:
                ...
                view.viewOrigin = CGPointMake(view.frame.origin.x+edgeWidth*cos(M_PI/3), view.frame.origin.y);
                view.viewSize = CGSizeMake(view.frame.size.width-edgeWidth*cos(M_PI/3), view.frame.size.height);
                if ((self.trianglePercent*view.frame.size.height < edgeWidth*cos(M_PI/3))||((1-self.trianglePercent)*view.frame.size.height < edgeWidth*cos(M_PI/3)))
                {
                    _isMargin = YES;
                }
                break;
...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    return [self initWithFrame:frame andDirection:direction andTitles:titles andImages:nil andView:nil trianglePecent:0.5];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    _cell = [tableView dequeueReusableCellWithIdentifier:reuseCell];
    if (_cell == nil) {
        _cell = [[UITableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseCell];
    }
    
    if (self.images.count > 0) {
        if (self.images.count - 1 >= indexPath.row) {
            _cell.imageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@",self.images[indexPath.row]]];
        }
        
    }
    if (self.popTintColor) {
        _cell.textLabel.textColor = self.popTintColor;
    }
    if (self.titles.count > 1) {
        _cell.textLabel.font = [UIFont systemFontOfSize:14];
        _cell.textLabel.text = _titles[indexPath.row];
    }
    return _cell;
}
